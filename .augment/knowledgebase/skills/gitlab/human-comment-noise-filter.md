---
name: gitlab-human-comment-noise-filter
description: Filter human MR comments down to those with meaningful team signal — keep only comments with 1+ reactions, substantive replies, or addressed change requests; drop LGTMs, process noise, and bot-generated traffic. Used by analyzers ranking human signal on an MR.
---
# Human comment noise filter

Apply this filter to every human (non-agent, non-bot) MR comment before recording it as signal. The goal is to retain only comments where the team itself indicated the comment was noteworthy.

**Keep a human comment if it meets at least one of:**

- **1+ emoji reactions** (thumbsup or thumbsdown) from other humans — indicates team agreement or disagreement.
...
- Bot-generated: CI pipeline status, coverage reports, automated check summaries.
- Process-only: `please rebase`, `waiting for pipeline`, `force-push when ready`.
